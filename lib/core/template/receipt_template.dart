...
class ReceiptTemplate {
  ...
  static Future<List<int>> ticket({TransactionModel? transaction}) async {
    ...
    bytes += generator.row(
      posColumn('Order ID', transaction?.referenceId ?? 'reference Id'),
    );
    ...
    if (transaction != null) {
      for (var element in transaction.items) {
        ...
        bytes += generator.row(
          posColumn(element.itemPrice.toIDR(), '${element.qty}x'),
        );
      }
    } else {
      bytes += title(generator, 'it should be your order tho.. :)');
      bytes += generator.row(
        posColumn('also this is your order price dawggg', '-x'),
      );
    }
    ...
    bytes += generator.row(
      posColumn(
        ...
        transaction?.amount.toIDR() ??
            'it should be your total amount order dayummmm :)',
        isBold: true,
      ),
    );
    ...
    bytes += generator.row(
      posColumn('Subtotal', transaction?.amount.toIDR() ?? 'Rp 44.800'),
    );
    bytes += generator.row(posColumn('Pajak', 0.toIDR()));
    bytes += generator.row(
      posColumn('Diskon', '- ${transaction?.discount.toIDR() ?? 0.toIDR()}'),
    );
    bytes += generator.hr();
    if (transaction != null) {
      bytes += generator.row(
    ...
    if (transaction != null) {
      bytes += generator.row(
        posColumn(
          'Total Kembali',
          (transaction.payAmount - (transaction.amount - transaction.discount))
              .toIDR(),
          isBold: true,
        ),
      );
